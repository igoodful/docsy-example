...
config.guess 是一个用于自动检测当前系统类型的 Shell 脚本，通常用于 GNU 构建系统（如 Autoconf）中，以生成与目标系统兼容的配置。自动检测当前系统的 三元组（Triplet），格式为 CPU-厂商-操作系统（如 x86_64-pc-linux-gnu）
...
```bash
2025-03-29T01:32:58 [mysql@ky10arm-98 /glc/bison/source/bison-3.8.2/build-aux] $ ./config.guess -v
GNU config.guess (2021-06-03)
...
2025-03-29T01:35:06 [mysql@ky10arm-98 /glc/bison/source/bison-3.8.2/build-aux] $ ./config.guess -h
...
2025-03-29T01:35:11 [mysql@ky10arm-98 /glc/bison/source/bison-3.8.2/build-aux] $ ./config.guess -t
2021-06-03
...
- 如果遇到不支持的平台，从 GNU 官网下载最新版：
示例：手动更新 config.guess 和 config.sub
```bash
...
```
这些文件共同构成了 GNU 构建系统的核心，确保软件能够适配多种平台和环境。
...
AViiON:dgux:*:*)
	...
	if test "$UNAME_PROCESSOR" = mc88100 || test "$UNAME_PROCESSOR" = mc88110; then
		if test "$TARGET_BINARY_INTERFACE"x = m88kdguxelfx ||
			test "$TARGET_BINARY_INTERFACE"x = x; then
...
EOF
		if $CC_FOR_BUILD -o "$dummy" "$dummy.c" && SYSTEM_NAME=$("$dummy"); then
			GUESS=$SYSTEM_NAME
		else
...
EOF
	...
	eval "$cc_set_vars"
	test "x$CPU" != x && {
		echo "$CPU${MIPS_ENDIAN}-unknown-linux-$LIBCABI"
		exit
	}
	;;
mips64el:Linux:*:*)
...
x86_64:Linux:*:*)
	...
	if test "$CC_FOR_BUILD" != no_compiler_found; then
		if (
			echo '#ifdef __ILP32__'
			echo IS_X32
			echo '#endif'
		) |
			(CCOPTS="" $CC_FOR_BUILD -E - 2>/dev/null) |
			grep IS_X32 >/dev/null; then
			LIBCABI=${LIBC}x32
		fi
	fi
